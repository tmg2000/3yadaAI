...
export async function seedAdmin() {
  ...
  const existing = await db.prepare("SELECT id FROM admins WHERE email = ?").get("admin@3yada.ai");
  if (existing) return;

  const hash = bcrypt.hashSync("admin123", SALT_ROUNDS);
  await db.prepare("INSERT INTO admins (id, name, email, password_hash, role) VALUES (?, ?, ?, ?, ?)").run(
    generateId(), "المشرف العام", "admin@3yada.ai", hash, "super_admin"
  );
}

export async function loginAdmin(email: string, password: string): Promise<{ admin: { id: string; name: string; email: string; role: string }; token: string }> {
  const db = await getDb();
  const row = await db.prepare("SELECT * FROM admins WHERE email = ?").get(email) as AdminRow | undefined;
...
export async function adminAuthMiddleware(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    ...
    const db = await getDb();
    const row = await db.prepare("SELECT id, name, email, role FROM admins WHERE id = ?").get(payload.adminId) as AdminRow | undefined;
    if (!row) {
