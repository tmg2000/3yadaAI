...
router.post("/", (req, res) => {
  ...
  const db = getDb();
  const doctor = db.prepare("SELECT available_slots FROM doctors WHERE id = ?").get(doctorId) as
  ...
  if (sid) {
    db.prepare("UPDATE sessions SET status = 'completed', updated_at = datetime('now') WHERE id = ? AND user_id = ?")
      .run(sid, req.user!.id);
  }
  ...
  if (sid) {
    const session = db.prepare("SELECT summary_json FROM sessions WHERE id = ? AND user_id = ?").get(sid, req.user!.id) as { summary_json: string } | undefined;
...
router.put("/:id/request-modify", (req, res) => {
  try {
    const db = getDb();
    const appt = db.prepare("SELECT * FROM appointments WHERE id = ? AND user_id = ?").get(req.params.id, req.user!.id) as any;
    if (!appt) {
      res.status(404).json({ error: "الموعد غير موجود" });
      return;
    }
    const { slot, patientName, phone } = req.body as Record<string, string>;
    if (slot) db.prepare("UPDATE appointments SET slot = ? WHERE id = ?").run(slot, req.params.id);
    if (patientName) db.prepare("UPDATE appointments SET patient_name = ? WHERE id = ?").run(patientName, req.params.id);
    if (phone) db.prepare("UPDATE appointments SET phone = ? WHERE id = ?").run(phone, req.params.id);
    db.prepare("UPDATE appointments SET status = 'modification_requested' WHERE id = ?").run(req.params.id);
