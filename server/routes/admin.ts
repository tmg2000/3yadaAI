...
router.get("/dashboard", adminAuthMiddleware, (req, res) => {
  ...
  const totalDoctors = (db.prepare("SELECT COUNT(*) as c FROM doctors").get() as any).c;
  ...
  const totalDoctorRevenue = (db.prepare("SELECT COALESCE(SUM(fee), 0) as c FROM appointments WHERE status = 'completed'").get() as any).c;
  const totalPlatformRevenue = (db.prepare(`
  ...
  `).get() as any).c;
  const verifiedDoctors = (db.prepare("SELECT COUNT(*) as c FROM doctors WHERE is_verified = 1").get() as any).c;
  const activeSubscriptions = (db.prepare("SELECT COUNT(*) as c FROM doctor_subscriptions WHERE active = 1 AND plan_id != 'plan_free'").get() as any).c;
  ...
  const lastMonthUsers = (db.prepare("SELECT COUNT(*) as c FROM users WHERE created_at >= ? AND created_at < ?").get(lastMonthStart.toISOString(), thisMonthStart.toISOString()) as any).c;
  const lastMonthDoctors = (db.prepare("SELECT COUNT(*) as c FROM doctors WHERE created_at >= ? AND created_at < ?").get(lastMonthStart.toISOString(), thisMonthStart.toISOString()) as any).c;
  const lastMonthAppts = (db.prepare("SELECT COUNT(*) as c FROM appointments WHERE slot >= ? AND slot < ?").get(lastMonthStart.toISOString(), thisMonthStart.toISOString()) as any).c;
  const lastMonthRevenue = (db.prepare("SELECT COALESCE(SUM(fee), 0) as c FROM appointments WHERE status = 'completed' AND slot >= ? AND slot < ?").get(lastMonthStart.toISOString(), thisMonthStart.toISOString()) as any).c;
...
// --- Admin Report ---
router.get("/report", adminAuthMiddleware, (req, res) => {
  try {
    const db = getDb();
    ...
    const totalUsers = (db.prepare("SELECT COUNT(*) as c FROM users WHERE created_at >= ?").get(start) as any).c;
    const totalDoctors = (db.prepare("SELECT COUNT(*) as c FROM doctors WHERE created_at >= ?").get(start) as any).c;
    const totalAppointments = (db.prepare("SELECT COUNT(*) as c FROM appointments WHERE slot >= ?").get(start) as any).c;
    const completedAppointments = (db.prepare("SELECT COUNT(*) as c FROM appointments WHERE status = 'completed' AND slot >= ?").get(start) as any).c;
    const totalRevenue = (db.prepare("SELECT COALESCE(SUM(fee), 0) as c FROM appointments WHERE status = 'completed' AND slot >= ?").get(start) as any).c;
    const totalConversations = (db.prepare("SELECT COUNT(*) as c FROM conversations WHERE created_at >= ?").get(start) as any).c;
    const activeDoctors = (db.prepare("SELECT COUNT(DISTINCT doctor_id) as c FROM appointments WHERE slot >= ?").get(start) as any).c;
    ...
    for (let i = 0; i < months; i++) {
      ...
      const mUsers = (db.prepare("SELECT COUNT(*) as c FROM users WHERE created_at >= ? AND created_at < ?").get(mStart.toISOString(), mEnd.toISOString()) as any).c;
      const mDoctors = (db.prepare("SELECT COUNT(*) as c FROM doctors WHERE created_at >= ? AND created_at < ?").get(mStart.toISOString(), mEnd.toISOString()) as any).c;
      const mAppts = (db.prepare("SELECT COUNT(*) as c FROM appointments WHERE slot >= ? AND slot < ?").get(mStart.toISOString(), mEnd.toISOString()) as any).c;
      const mRev = (db.prepare("SELECT COALESCE(SUM(fee), 0) as c FROM appointments WHERE status = 'completed' AND slot >= ? AND slot < ?").get(mStart.toISOString(), mEnd.toISOString()) as any).c;
...
// --- Admins management ---
router.get("/admins", adminAuthMiddleware, (req, res) => {
  const db = getDb();
...
router.post("/admins", adminAuthMiddleware, async (req, res) => {
  try {
    ...
    db.prepare("INSERT INTO admins (id, name, email, password_hash, role) VALUES (?, ?, ?, ?, 'admin')").run(
...
router.delete("/admins/:id", adminAuthMiddleware, (req, res) => {
  try {
    const db = getDb();
...
router.put("/admins/:id/password", adminAuthMiddleware, async (req, res) => {
  try {
    ...
    const row = db.prepare("SELECT password_hash FROM admins WHERE id = ?").get(admin.id) as any;
...
router.get("/conversations", adminAuthMiddleware, (req, res) => {
  ...
  const conversations = db.prepare(dataQuery).all(...dataParams);
  res.json({ conversations, total, page, limit });
});

// Grouped conversations by user (MUST be before /:id route)
router.get("/conversations/grouped", adminAuthMiddleware, (req, res) => {
  const db = getDb();
...
router.get("/conversations/:id", adminAuthMiddleware, (req, res) => {
  const db = getDb();
...
router.get("/doctors/export", adminAuthMiddleware, (req, res) => {
  try {
    const db = getDb();
...
router.get("/doctors/:id", adminAuthMiddleware, (req, res) => {
  try {
    const db = getDb();
...
router.get("/doctors", adminAuthMiddleware, (req, res) => {
  const db = getDb();
...
// Update doctor
router.put("/doctors/:id", adminAuthMiddleware, (req, res) => {
  try {
    const db = getDb();
    ...
    if (name !== undefined) db.prepare("UPDATE doctors SET name = ? WHERE id = ?").run(name, req.params.id);
    if (specialty !== undefined) db.prepare("UPDATE doctors SET specialty = ? WHERE id = ?").run(specialty, req.params.id);
    if (hospital !== undefined) db.prepare("UPDATE doctors SET hospital = ? WHERE id = ?").run(hospital, req.params.id);
    if (city !== undefined) db.prepare("UPDATE doctors SET city = ? WHERE id = ?").run(city, req.params.id);
    if (area !== undefined) db.prepare("UPDATE doctors SET area = ? WHERE id = ?").run(area, req.params.id);
    if (consultationFee !== undefined) db.prepare("UPDATE doctors SET consultation_fee = ? WHERE id = ?").run(consultationFee, req.params.id);
    if (isVerified !== undefined) db.prepare("UPDATE doctors SET is_verified = ? WHERE id = ?").run(isVerified ? 1 : 0, req.params.id);
    if (acceptedInsurances !== undefined) db.prepare("UPDATE doctors SET accepted_insurances = ? WHERE id = ?").run(JSON.stringify(acceptedInsurances), req.params.id);
    if (licenseNumber !== undefined) db.prepare("UPDATE doctors SET license_number = ? WHERE id = ?").run(licenseNumber, req.params.id);
    if (phone !== undefined) db.prepare("UPDATE doctors SET phone = ? WHERE id = ?").run(phone, req.params.id);
    if (bio !== undefined) db.prepare("UPDATE doctors SET bio = ? WHERE id = ?").run(bio, req.params.id);
    if (email !== undefined) db.prepare("UPDATE doctors SET email = ? WHERE id = ?").run(email, req.params.id);
    res.json({ success: true });
  } catch (err) {
...
router.post("/doctors/:id/subscription", adminAuthMiddleware, (req, res) => {
  try {
    const db = getDb();
    ...
    db.prepare("UPDATE doctor_subscriptions SET active = 0 WHERE doctor_id = ? AND active = 1").run(req.params.id);
...
router.put("/doctors/:id/password", adminAuthMiddleware, async (req, res) => {
  try {
    ...
    const db = getDb();
    const hash = bcrypt.hashSync(password, SALT_ROUNDS);
    db.prepare("UPDATE doctors SET password_hash = ? WHERE id = ?").run(hash, req.params.id);
    res.json({ success: true });
  } catch (err) {
    res.status(400).json({ error: err instanceof Error ? err.message : "فشل تغيير كلمة المرور" });
  }
...
// Delete doctor
router.delete("/doctors/:id", adminAuthMiddleware, (req, res) => {
  try {
    const db = getDb();
...
// --- Patients ---
router.get("/patients", adminAuthMiddleware, (req, res) => {
  const db = getDb();
...
// Patient detail
router.get("/patients/:id", adminAuthMiddleware, (req, res) => {
  const db = getDb();
  const patient = db.prepare("SELECT id, name, email, phone, date_of_birth, gender, created_at FROM users WHERE id = ?").get(req.params.id) as any;
  if (!patient) {
...
// --- Appointments ---
router.get("/appointments", adminAuthMiddleware, (req, res) => {
  const db = getDb();
...
router.put("/appointments/:id", adminAuthMiddleware, (req, res) => {
  try {
    const db = getDb();
    ...
    if (slot !== undefined) db.prepare("UPDATE appointments SET slot = ? WHERE id = ?").run(slot, req.params.id);
    if (doctorName !== undefined) db.prepare("UPDATE appointments SET doctor_name = ? WHERE id = ?").run(doctorName, req.params.id);
    if (patientName !== undefined) db.prepare("UPDATE appointments SET patient_name = ? WHERE id = ?").run(patientName, req.params.id);
    if (phone !== undefined) db.prepare("UPDATE appointments SET phone = ? WHERE id = ?").run(phone, req.params.id);
    if (fee !== undefined) db.prepare("UPDATE appointments SET fee = ? WHERE id = ?").run(fee, req.params.id);
    res.json({ success: true });
  } catch (err) {
    res.status(400).json({ error: err instanceof Error ? err.message : "فشل التحديث" });
  }
...
// --- Subscriptions ---
router.get("/subscriptions", adminAuthMiddleware, (req, res) => {
  const db = getDb();
...
router.get("/subscription-plans", adminAuthMiddleware, (_req, res) => {
  const db = getDb();
